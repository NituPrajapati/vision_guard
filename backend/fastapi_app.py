...
async def detect(file: UploadFile = File(...), request: Request = None):
    try:
        ...
        # Log if no objects detected
        if len(detected_labels) == 0:
            logger.info(f"No objects detected in detection.")
...
def _run_live_detection(cam_index: int = 0, user_email: str = None):
    ...
    while is_live_running and cap.isOpened():
        ...
        if current_time - _last_snapshot_time >= _snapshot_interval and user_email:
            try:
                ...
                # Log if no objects detected
                if len(detected_labels) == 0:
                    logger.info(f"No objects detected in live detection snapshot")
    ...
    if user_email and os.path.exists(out_path):
        try:
            ...
            # Log if no objects detected in final snapshot
            if len(last_detected_labels) == 0:
                logger.info(f"No objects detected in final live detection snapshot")
